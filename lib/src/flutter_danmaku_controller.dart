...
class FlutterDanmakuController {
  ...
  AddBulletResBody addDanmaku(String text,
    ...
    // 寻找可用的轨道
    FlutterDanmakuTrack? track = _findAvailableTrack(bulletSize, bulletType: bulletType, position: position, offsetMS: (offsetMS ?? 0));
    // 如果没有找到可用的轨道
    if (track == null)
      return AddBulletResBody(
        AddBulletResCode.noSpace, message: '',
      );
    FlutterDanmakuBulletModel bullet = _bulletManager.initBullet(text, track.id, bulletSize, track.offsetTop,
        prevBulletId: track.lastBulletId ?? UniqueKey(), position: position, bulletType: bulletType, color: color, builder: builder, offsetMS: (offsetMS ?? 0));
    if (bulletType == FlutterDanmakuBulletType.scroll) {
      ...
      if (position == FlutterDanmakuBulletPosition.any) {
        track.loadFixedBulletId(bullet.id ?? UniqueKey());
      }
    }
    return AddBulletResBody(AddBulletResCode.success, data: bullet.id, message: '');
  }
  ...
  FlutterDanmakuTrack? _findAllowInsertBottomTrack(Size bulletSize) {
    ...
    for (int i = tracks.length - 1; i >= tracks.length - 3; i--) {
      // 从当前的弹幕里找 有没有在这个轨道上的
      FlutterDanmakuBulletModel bullet = _bulletManager.bottomBullets.firstWhere((element) => element.trackId == tracks[i].id);
      if (bullet == null) {
        _track = tracks[i];
        break;
      }
